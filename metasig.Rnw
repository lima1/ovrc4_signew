...
are shown in Supplemental Figure~\ref{fig:km:tcgasubtypes}. Best patient
stratifications were observed in the Immunoreactive subtype.
...
X <- esets.f$TCGA_eset[, !is.na(esets.f$TCGA_eset$subtype)]
genesets <- .getFingerprintGeneSets(X, as.factor(X$subtype), p.value=0.001,
...
dfrc$subtype <- 
  unlist(sapply(gsva.res[-8], function(x) apply(x,2,which.max)))
...
dfrc$subtype[unlist(lapply(gsva.res[-8], function(x) apply(x,2,max))) < 0.1] <- NA

dfrcval$subtype <- 
  unlist(sapply(gsva.res.val, function(x) apply(x,2,which.max)))
dfrcval$subtype[unlist(lapply(gsva.res.val, function(x) apply(x,2,max))) < 0.1] <- NA
@
<<accuracytcga>>=
idx <-  !is.na(esets.f$TCGA_eset$subtype)
x <- table(apply(gsva.res[[8]],2,which.max)[idx], X$subtype)
x <- round(sum(sapply(1:4, function(i) x[i,i]))/ncol(X)*100,digits=1)
@
...
classified \Sexpr{x}\% of the samples correctly. In Supplemental
...
TCGA and the remaining datasets the best prognosis. 
...
dfrcboth$subtype<- as.factor( levels(esets.f[[8]]$subtype)[dfrcboth$subtype] )
@

\begin{figure}
<<subtypesosdiff,fig.width=10,fig.height=5,out.width="0.96\\textwidth",out.height="0.48\\textwidth">>=
par(mar=c(5,5,3,2), mfrow=c(1,2))
plotKM(y=dfrcboth$y, strata=dfrcboth$subtype,
censor.at=365.25*5,main="A)  All datasets except TCGA")
plotKM(y=esets.f$TCGA_eset$y, strata=esets.f$TCGA_eset$subtype,
censor.at=365.25*5,main="B)  TCGA")
@
\caption{Association of subtype and overall survival.
(A) All training and validation datasets excluding TCGA. (B)
Stratification of TCGA samples by subtype.}
...
par(mfrow=c(2,2))
for (i in 1:4) {
subtype <- levels(esets.f$TCGA_eset$subtype)[i]
idx <- dfrcboth$subtype== subtype
...
strata=strata_we[idx,1],censor.at=365.25*5,
        ...
        cex.base=1.4,main=subtype)
}        
@
\caption{Kaplan-Meier analysis of our overall survival signature as in Figures
1 and 3 of the main paper, but stratified by subtype as in Supplemental
...
We generated a gene signature for suboptimal debulking surgery and
...
curves. In Supplemental Table~\ref{ tbl:reg:debulking }, we show the logistic
regression coefficients of the gene signature risk score adjusted for FIGO
stage (III vs. IV).
...
idx <- !sapply(Xs.alls34,.debulkingFilter)
preds.logit <- .p2logit(Xs.alls34[idx], fits=res.debulking.s34$fits[idx])
aucs.db <- .plotROCpanel(preds[idx],labels[idx],titles[idx],3,3)
@
...
outreg(list(fit.debulking), modelLabels=c("Meta-Analysis"),
title="Prediction of debulking status. The table lists the
regressions for our leave-one-dataset-out cross-validated meta-analysis
debulking gene signature
signature. The predictions were adjusted for tumor stage.",
label="tbl:reg:debulking", tight=FALSE,
varLabels=list(risk="Gene Signature",tumorstage="Stage (III vs IV)"))
@
