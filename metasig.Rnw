...
% Repeat the leave-one-dataset-out cross-valiation, but use only the n
% smallest/largest datasets for training
...
p <-ggplot(df.hr, aes(n,HR))+geom_smooth(method="lm")+geom_point(aes(colour=containstcga))+facet_wrap(~Dataset)+coord_cartesian(ylim = c(0.75, 3))+ylab("Hazard Ratio")+xlab("Total Training Sample Size")+scale_colour_discrete(name = "TCGA used for training")+theme(axis.text.x=element_text(angle=45,hjust=1))
@
