...
<<stratification>>=
# Calculate accuray of risk stratification
x <- ihc$pSmad23 + ihc$POSTN + ihc$CXCL14
idx <- !is.na(x)
p <- c(counts[1]*2/3, counts[1]+counts[2]*1/3)
table(as.factor(sapply(x[idx], function(y) ifelse(y <= cutoff[1], "low",
ifelse(y <= cutoff[2], "mid", "high")))), ihc$Debulking[idx])
@
