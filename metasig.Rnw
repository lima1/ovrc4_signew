...
strata_jci <-   do.call(rbind, lapply(c(res.lodo.jci,res.val.jci[-4]), function(x)
data.frame(x$strata, x$y)))
...
plotKM(y=Surv(strata_jci[,2][,1],strata_jci[,2][,2]),
strata=strata_jci[,1],censor.at=365.25*5,
        show.n.risk=FALSE,show.legend=FALSE,show.HR=TRUE,cex.HR=1.1,
        show.PV=FALSE,
        xlab="Time (Days)",ylab="Overall Survival (%)",
        cex.base=1.4,main="B)   JCI Signature")
