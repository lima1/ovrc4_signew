...
<<removeyoshihara2>>=
# there is some overlap in samples
X <- esets.f$GSE17260_eset 
Y <- esets.validation$GSE32062.GPL6480_eset
isc <- intersect(gsub("s","", X$alt_sample_name), gsub("d.*","", Y$alt_sample_name))
iscx <- sapply(isc, function(x) grep(paste("^",x,"d",sep=""),
 Y$alt_sample_name))

esets.validation$GSE32062.GPL6480_eset <- esets.validation$GSE32062.GPL6480_eset[,-iscx]
esets.uf$GSE32062.GPL6480_eset <- esets.uf$GSE32062.GPL6480_eset[,-iscx]
esets.debulking$GSE32062.GPL6480_eset <- esets.debulking$GSE32062.GPL6480_eset[,-iscx]
...
<<cached_medium_learnsubtypegenesets, cache=TRUE>>=
...
authors. Numbers in brackets are standard errors.",
...
.filterfx <- function(eset) { 
    ncol(eset)==129 | .defaultFilter(eset)
}
