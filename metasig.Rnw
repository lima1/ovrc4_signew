...
<<changedebulkingtcga>>=
# remove Bonome samples, change debulkig definition
...
Supplemental Figure~\ref{fig:km:tcgasubtypesother}, in contrast, shows the
patient stratification in inferred TCGA subtypes in all other datasets.

\begin{figure}[!htb]
<<kmsubtypes, results='hide',out.width="0.7\\textwidth",out.height="0.7\\textwidth">>=
# fetch the unbiased risk scores from the leave-one-dataset-out
# cross-validation for TCGA
...
# now generate a forest plot for the unbiased risk scores
...
plotKM(y=dfrcboth[idx,]$y,
strata=dfrcboth[idx,]$strata,censor.at=365.25*5,
...
4A-D of the main paper, but excluding the TCGA data here, showing consistent
results with the TCGA data in Supplemental Figure~\ref{fig:km:tcgasubtypes}.
Point of this figure is to show that in our own stratification of datasets in
TCGA subtypes, we see similar trends as we see in the official TCGA data and
subtypes, with the only exception being the proliferative subtype.}
...
coefs.debulking.s34b <- metaCMA.coefs(Xs.alls34b, y="debulking", family=binomial)
res.debulking.s34b  <-
metaCMA(Xs.alls34b,y="debulking",coefs=coefs.debulking.s34b, n=200,
filter.fun=.debulkingFilter)
tmp <- metaCMA.opt(Xs.alls34b, coefs=coefs.debulking.s34b,n=200,y="debulking")
final.model.debulking.bonome <- tmp$model
@
...
labels <- lapply(Xs.alls34b, function(X) X$debulking)
titles <- .getDatasetNames(Xs.alls34b)
idx.debfi <- !sapply(Xs.alls34b,.debulkingFilter)
@
...
#table(unlist(xxx[[2]]), unlist(labels[idx.debfi]))
...
\begin{figure}
<<cached_short_debulking_berchuck04, cache=TRUE,fig.width=9.5,fig.height=6,out.width="0.95\\textwidth",out.height="0.6\\textwidth",results='hide'>>=
...
preds.berchuck04 <- lapply(Xs.alls34b, function(X) predict(model.berchuck04, newdata=X)@lp)
 xxx <- .forestPlotDebulking(preds1=preds.berchuck04[idx.debfi][-7],
  labels=labels[idx.debfi][-7],
 prior=priors[idx.debfi][-7],titles=titles[idx.debfi][-7])
@
...
probesets <- do.call(cbind, lapply(Xs.alls34b, function(X)
 as.character(featureData(X)[sig,]$maxmean_probeset)))
...
dfrc.debulking <- .createClinical(Xs.alls34b)
...
dfrc.debulking$risk <- unlist(lapply(res.debulking.s34b$fits, function(X)
X$risk@lp))

dfrc.debulking$risk_os <-  unlist(lapply(names(Xs.alls34b), function(id)  if
    (id %in% names(ma[[10]]$fits)) predict(ma[[10]]$fits[[id]]$fit,
        newdata=Xs.alls34b[[id]])@lp else predict(final.model,
        newdata=Xs.alls34b[[id]])@lp) )
...
success \cite{Berchuck:2004} we were aware of, with the corresponding forest
plot shown in Supplemental Figure~\ref{fig:forest:debulking:berchuck04}. Genes
of the signature were converted to current HGNC symbols and risk scores were
calculated by averaging the scaled expression values: the expression average of
genes down-regulated in suboptimal was subtracted from the average of
up-regulated genes. The AUC of the Berchuck signature was significantly smaller
($P < \Sexpr{round(aucdiff$p.value,2)}$, \cite{DeLong:1988}) than the AUC of
our meta-analysis signature.
...
resM2 <- .getFoldChanges(esets.db, final.model.debulking.bonome)


write.csv(resM, "output/final_signature_debulking_foldchanges.csv")
write.csv(resM2, file="output/signature_debulking_without_bonome.csv")
@
...
<<tcgasubtypescombined,fig.width=7,fig.height=4.7,out.width="0.9\\textwidth",out.height="0.6\\textwidth",results='hide',include=FALSE>>=
...
dfrc.subtypes$strata <- c(dfrcboth$strata, unlist(sapply(strata.subtypes, function(x)
  x$strata)))
dfrc.subtypes$strata <- as.factor( levels(dfrcboth$strata)[
...
eset.qrtpcr <- ExpressionSet(sqrt(impute.knn(M)$data),
...
auc(eset.qrtpcr$debulking, predict(model.qrtpcr,newdata=eset.qrtpcr)@lp)

.plotROC(predict(model.qrtpcr,newdata=eset.qrtpcr)@lp,eset.qrtpcr$debulking)
@
