...
texreg(list(fit.debulking,fit.debulking.berchuck04), model.names=c("Meta-Analysis", "Berchuck et al. 2004"),
...
and stage. We further validated 8 genes by qRT-PCR in 78 Bonome samples not used for training. In
Figure~\ref{fig:qrtpcr:cor}, we show the correlation of the qRT-PCR expression
...
.plotROC(ihc$CXCL14, as.factor(ihc$Debulking))
@

<<ihcmultivariate,fig.width=3.5,fig.height=3.5,include=FALSE>>=
.plotROC(ihc$POSTN+ihc$pSmad2+ihc$CXCL14, as.factor(ihc$Debulking))
@
...
texreg(list(fit, fit.adj, fit.POSTN, fit.pSmad23,
    fit.CXCL14),model.names=c("IHC", "IHC adj.", "POSTN adj.", 
