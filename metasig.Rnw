...
respectively (corresponding to the 3 sections of the Venn diagram). Genes with
significant heterogeneity are marked in black in the heatmap.  In the
...
signature. This was expected, since surgery outcome is not a clearly
...
Accounting for heterogeneity with a random-effects model would overweight
