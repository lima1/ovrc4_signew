...
\title{Risk Prediction for late-stage ovarian cancer by meta-analysis of 1,622
patient samples - Supplemental Information}

\author{Markus Riester, Wei Wei, Levi Waldron, Aedin Culhane, Lorenzo Trippa,
\\ Franziska Michor, Curtis Huttenhower, Esther Olivia, Sung-hoon Kim,\\
...
validation, we tested our models in a leave-one-dataset-out cross-validation
procedure (Supplemental Figure~\ref{fig:flowchart}). The goal of this
procedure is to build the best possible model by using all major datasets for
training, and to still obtain an accurate (in particular not over-optimistic)
estimate of how well this model will perform applied to independent data. 

For the overall survival signature, we first selected the 6 largest datasets
as training datasets. More exactly, we excluded the small datasets mostly
because small datasets often display unusual patient characteristics. Then we
trained 6 different models and always excluded one dataset from training.
These 6 models were then always validated on the corresponding excluded
datasets. This procedure thus results in one risk prediction per patient,
which can then be used for model evaluation by comparing the risk prediction
with the observed outcome. This is not an over-optimistic evaluation,
because to calculate a particular patient's risk score, only information from
different patient cohorts was utilized. 
...
that smaller signatures tend to be less robust than large signatures.
Furthermore, larger signatures allow the use of pathway enrichment tools,
which are often useful for prioritizing genes for further experimental validation.
Finally, because we plan to apply the models to a large cohort of FFPE
specimens, we decided for a larger signature to provide some redundancy.  This
is important as we expect that some genes in the signatures will not be
available after quality controlling.
...
Note that increasing the number of features in an ensemble classifier such as
the compound covariate score is not the same as overfitting in multivariate
regression, which refers to tuning a model until it perfectly explains the
data. All tuning parameters - algorithmic details, training datasets and gene
...
We used standard bootstrap to estimate the statistical significance of the
observed HR differences of our signature compared to TCGA. We considered all
cohorts excluding TCGA (\Sexpr{nrow(dfrcboth)} patients where direct
comparison to the TCGA signature could be made) in this procedure. We
generated 10,000 bootstrap replicates in which \Sexpr{nrow(dfrcboth)} patients
were sampled with replacement from this pool of \Sexpr{nrow(dfrcboth)}
patients. Confidence intervals were then estimated from this empirical
distribution of Hazard Ratio differences. The reported p-value corresponds to
the fraction of bootstrap samples with higher HR in TCGA. Note that the HR
confidence intervals of the two signatures do not need to be non-overlapping
...
Verhaak et al. proposed a multi-variate model including tumor stage,
...
multivariate model using the CLOVAR patient stratifications in high- and
...
the multivariate predictions) are shown in Supplemental Figure~\ref{fig:cor:risk}. 
In this figure, we further compare the models with a random-effects variant of
...
coefs=coefs.debulking.s34b,n=200,y="debulking", filter.fun=.debulkingFilter)
...
ma.db <- lapply(c(5,10,seq(25,250,25)), function(i)
    metaCMA(esets.debulking.f,coefs=coefs.debulking.s34b,n=i,y="debulking",filter.fun=.debulkingFilter))
@

<<cached_short_debulking_finalmodelall, cache=TRUE>>=
tmpall.db <- metaCMA.opt(esets.debulking.f,
coefs=coefs.debulking.s34b,n=nrow(esets.debulking.f[[1]]),y="debulking", filter.fun=.debulkingFilter)
@
...
stratifications (Figure 4a) and of a multivariate model utilizing these
...
log-transformed Hazard Ratios) for the multivariate models. For datasets with
...
caption="Cox regression coefficients for multivariate models utilizing the
...
# remove the Yoshihara 2012 data to get the 6 datasets we used for training
.filterfx <- function(eset) { 
    pubMedIds(eset)=="22241791" | .defaultFilter(eset)
}
...
#p <- p+ scale_shape(solid=FALSE, name = "Only Training Datasets")
#p <- p+ scale_colour_discrete( name = "Only Training Datasets")
...
\begin{figure}
<<plotdatasetshr>>=
facetAdjust(p+theme_classic2(14)+theme(axis.text.x = element_text(angle = 45, hjust = 1)))
@
...
the y-axis. This analysis showed that increasing the sample size via
meta-analysis typically increased the model HRs. TCGA data was further
identified as an extremely difficult validation dataset.}
...
%<<cached_medium_debulking_rankprod, cache=TRUE, results='hide'>>=
%res.debulking.rp <- metaCMA.rankproduct(esets.debulking.f[idx.debfi], y="debulking",
%treatment.label="suboptimal", huge=TRUE)
%@

%tmp.all <- metaCMA.opt(esets.debulking.f,
%coefs=coefs.debulking.s34b,n=nrow(esets.debulking.f[[1]]),y="debulking", filter.fun=.debulkingFilter)
...
the probability cutoffs. AUCs significantly ($P < 0.05$) larger than 0.5 are
...
Scoring of the intensity (protein level) of each antigen was conducted by
determining the percentage and intensity of positive cells in three different
areas at 100x magnification for each tissue core. First, the percentage of
positive cells in each section was scored with a 5-point scale: 0 for <5\%, 1
for 5-25\%, 2 for 26- 50\%, 3 for 50-75\%, and 4 for over 75\%.  Second, the
intensity of positive signal was scored with a 3-point scale: 1 for weak
staining, 2 for moderate staining, and 3 for intense staining. The weighed
score of staining intensity of each section was obtained by multiplying the
percentage score by the intensity score (the maximum weighed score is
12).Considering that each sample was represented by duplicate cores, the
average score was calculated for the IHC intensity of each sample (listed in
Table S1). A categorical system was then applied as follows: Grade 0 (-) score
0-3; Grade 1 (+) score 4-6; Grade 2 (++) score 7-9; Grade 4 (+++) score 9-12.
The IHC was performed by two individuals including one pathologist for
...
and stage. In the multivariate models with all three proteins, both adjusted
and unadjusted for stage and grade (shown in columns 1 and 2 of Supplemental
Table~\ref{tbl:coefficients:ihc}), inclusion of CXCL4 could not further
improve the model.

We further validated 8 genes by qRT-PCR in 78 Bonome samples not used for
training.  The selection criteria were (i) high rank by fold-change and FDR
and (ii) the results of our pathway analyses.  For example, PTCH1 was selected
because a validation would demonstrate another layour of evidence of the
hyperactivation of the Hedgehog/GLI pathway in suboptimal tumors.

In Figure~\ref{fig:qrtpcr:cor}, we show the Spearman correlation of the
...
<<ihcmultivariate,fig.width=3.5,fig.height=3.5,include=FALSE>>=
...
texreg(list(fit, fit.adj, fit.POSTN, fit.pSmad23,
    ...
    caption=paste("Multivariate prediction of debulking status.", 
    "Shown are multivariate models based on IHC staining only and",
...
the y-axis the qRT-PCR level. The Spearman correlation of platforms was highly
significant for all genes ($P < 0.001$).}
...
write.table(tmp[order(abs(tmp$Weighted.Mean), decreasing=TRUE),],
 file="output/final_signature_debulking.tsv",row.names=FALSE,sep="\t")
@
\clearpage
\section{Role of POSTN in ovarian cancer}

The biological rationale underlying the debulking signature is that the
metastatic potential of the tumor contributes to the difficulty of the
cytoreduction. POSTN positively drives each step of tumor metastasis
(migration, invasion, angiogenesis, formation of the metastatic niche, and the
growth of the micrometastases). 

Even with an extraordinarily large tumor burden, the confinement of the tumor
to the abdominal cavity makes the tumor resection efficacious in ovarian
cancer. Therefore, optimal debulking is more likely to be achieved in cancers
that are inherently less disseminative. Enhanced tumor metastasis may increase
the chance of surgical inaccessible tumors and thus lower the rate of optimal
debulking. The debulking signature developed in this study reveals the
co-activation of TGF-$\beta$, Ras/MAPK/EGR-1 and Hedgehog pathways as well as
stromal (TAF) activation in suboptimal debulked tumors. These changes may
directly contribute to malignant phenotypes with elevated EMT, extracellular
matrix remodeling and pro-angiogenic activity to facilitate tumor
dissemination (especially parenchymal metastases) and thereby render the
hindrance for surgical resection.
...
neutralizing antibody mediated significant reduction of tumor burden and the
...
plotKM(y=dfrcboth$y, strata=dfrcboth$strata,
        censor.at=365.25*5,
        show.n.risk=TRUE,show.legend=FALSE,show.HR=TRUE,cex.HR=1.1,
