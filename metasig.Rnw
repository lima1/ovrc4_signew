...
over-optimistic) estimate of how well this model will perform applied to
independent data. 
...
with the observed outcome. This is not an over-optimistic evaluation, because
to calculate a particular patient's risk score, only information from
...
coefficients from the microarray-based signature. That means all genes were
