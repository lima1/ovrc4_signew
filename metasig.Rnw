...
<<cached_medium_unocdiff, cache=TRUE>>=
# calculate Uno's C difference between our signature and TCGA
...
improved moderately (\Sexpr{.formatDC(res.c.delta.tcga[3,])}). 
...
zl <- cvRisk(fit,data=df1yr,y=df1yr$os_1yr,linear=FALSE)    
opt.adj <- .optimalROCclassification(zl$risk, zl$y)    
@
