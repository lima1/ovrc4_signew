...
patient samples: Biologic and Clinical Correlations - Supplemental Information}

\author{Markus Riester, Wei Wei, Levi Waldron, Aedin Culhane, Lorenzo Trippa, \\
Franziska Michor, Curtis Huttenhower, Giovanni Parmigiani, Michael Birrer}
\maketitle
...
with the overserved outcome. This is not an over-optimistic evaluation,
...
\caption{Flowchart leave-one-dataset-out cross-validation.}
\label{fig:flowchart}
\end{figure}
...
which is often useful to prioritize genes for further experimental validation.
...
is important because we expect that some genes in the signatures will not be
...
custom.coefs.names=c("Intercept", "Gene Signature","Stage (III vs IV)"), 
