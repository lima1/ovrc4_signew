...
fig.show='hold',warning=FALSE, echo=FALSE, autodep=TRUE)
...
represents the prediction concordance of a model with $x$ genes in the
...
the TCGA prediction models as described in \cite{TCGA:2011,Verhaak:2013}. In addition to the
...
overlapped with the TCGA signature ($P < 0.001$).  The p-value of this
...
The TCGA project recently published a slightly improved gene signature called
CLOVAR \cite{Verhaak:2013}. The correlation of the risk scores of our
meta-analysis signature and both TCGA signatures are shown in Supplemental
Figure~\ref{fig:cor:risk}. Verhaak et al. proposed a multi-variate model including
tumor stage, debulking status, BRCA1/2 mutation status and ssGSEA scores for
the immunoreactive and mesenchymal subtypes. BRCA1/2 mutation status was
unavailable for our validation datasets. As the survival association of the
ssGSEA scores were discovered in most of our validation datasets, we tested
a multivariate model using the CLOVAR patient stratifications in high- and
...
combined. This model performed very similar to the one proposed by the authors
using only 2 ssGSEA scores, but did not require any biased feature selection.
...
prior.short <- lapply(labels, function(x) sum(x=="short")/length(x))
...
<<optimalcutpoints>>=
ret <- .optimalROCclassification(preds[idx], labels[idx], best.method="closest.topleft")
@
...
\Sexpr{.formatDC(aucs$rma[c(1,2,5,6)])}, compared to 
\Sexpr{.formatDC(aucs_tcga$rma[c(1,2,5,6)])} for the TCGA signature. 
...
<<debulkingmetas34b, cache=TRUE, results='hide'>>=
...
\begin{figure}
<<kmplotstrain,cache=TRUE,fig.width=7,fig.height=4.7,out.width="0.9\\textwidth",out.height="0.6\\textwidth",results='hide'>>=
...
\begin{figure}
<<validationexternal,cache=TRUE,results='hide'>>=
...
\begin{figure}
\subfigure[]{
<<comptcga,fig.width=5.33,fig.height=8,out.width="0.467\\textwidth",out.height="0.7\\textwidth",results='hide'>>=
...
\subfigure[]{
<<fpcomptcga,fig.width=7.75,fig.height=11.5,out.width="0.4\\textwidth",out.height="0.7\\textwidth",results='hide'>>=
