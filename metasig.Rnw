...
In this section, we show the patient risk stratifications in the 4 TCGA
subtypes separately.  First we validated a prediction model that was trained
on all training datasets except TCGA in the 4 TCGA subtypes separately.
Kaplan-Meier plots for this analysis are shown in Supplemental
Figure~\ref{fig:km:tcgasubtypes}.  Supplemental
Figure~\ref{fig:km:tcgasubtypesother}, in contrast, shows the patient
stratification in inferred TCGA subtypes in all other datasets.
...
then validated in TCGA in the 4 subtypes suggested by the TCGA project separately.}
...
curves shown in Supplemental Figure~\ref{fig:roc:debulking:berchuck04}. Genes
