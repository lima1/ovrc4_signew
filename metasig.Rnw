...
preds.stage <- lapply(Xs.all[idx], function(X) X$tumorstage)
...
aucs.dbstage <- .plotROCpanel(preds.logit.stage,labels[idx],titles[idx],3,3)
@
...
\begin{figure}
<<plotfpdebulking,fig.width=15,fig.height=5.7,out.width="0.9\\textwidth",out.height="0.342\\textwidth",results='hide'>>=
priors <- sapply(labels, function(l) sum(l=="suboptimal")/length(l))
idx.stage <- sapply(preds.stage, function(x) sum(x==4,na.rm=TRUE)>0)
 xxx <- .doTestAllPair(preds1=preds.logit[idx.stage],
 preds2=preds.stage[idx.stage], labels=labels[idx][idx.stage],
 prior=priors[idx][idx.stage],titles=titles[idx][idx.stage])
@
