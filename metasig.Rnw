...
.getScores <- function(dataset, field="alt_sample_name", subtype="Mesenchymal") {
    ...
    if (field == "id") {
        ids <- match(x[idx.v,1], sampleNames(esets.f[idx.df]))
    } else {
        ids <- match(tolower(x[idx.v,1]), gsub("ovarian cancer ", "", tolower(dfrcboth[idx.w, field])))
    }

    tmp <- cbind(dfrcboth[idx.w,][ids,subtype], x[idx.v, paste(subtype, ".ssGSEA.normalized.score", sep="")])
    tmp <- tmp[complete.cases(tmp),]
    list(data.frame(tmp, Subtype=subtype), cor(tmp[,1], tmp[,2]))
}

d.f.verhaak <- do.call(rbind,unlist(lapply(c("Mesenchymal", "Immunoreactive"), function(subtype) list(.getScores("Bonome", field="alt_sample_name", subtype)[[1]], .getScores("Tothill", field="id", subtype)[[1]], .getScores("Yoshihara", field="id", subtype)[[1]])), recursive=FALSE))
...
corresponds to Figure 5B of the Tothill study, with the difference that here we
show only the late-stage, high-grade, serous tumors used in our meta-analysis.
(D) This scatterplot shows the high correlation of both the Mesenchymal and
Immunoreactive ssGSEA subtype scores from our implementation and the official
scores we extracted from the Verhaak et al. Supplement. The Mesenchymal and
Immunoreactive scores are utilized in the Verhaak et al. model. Shown are the scores
for the Tothill, Bonome, and Yoshihara samples.}
