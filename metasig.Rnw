...
compound covariate score is not prone to the overfitting as in multivariate
...
\label{sec:fixed}
In this section, we demonstrate how two different options for pooling the
regression coefficients across datasets affect the final overall survival
model. We compared a simple fixed-effects meta-analysis, in which coefficients
are weighted by the inverse of their squared standard errors, with a
random-effects model. For the random-effects model fitting, we used the
...
The choice of the meta-analysis method had no significant impact on the pooled
Cox regression coefficients; the \Sexpr{nunion} genes utilized in the two
signatures had highly similar coefficients in a fixed- and random-effects
...
coefficients in all datasets from the genes present in only the fixed-effects,
only the random-effects and present in both signatures, respectively
(corresponding to the 3 sections of the Venn diagram). Genes
with significant heterogeneity are marked in black in the heatmap.  In the
fixed-effects signature, \Sexpr{nhet} genes displayed significant
...
replaced in the random-effects signature with probesets displaying little
...
xx4 <- esets.f[idx][[4]]
xx6 <- esets.f[idx][[6]]
xxx <- mergeExprs(xx1,xx2,xx4,xx6)
...
Yoshihara 2010 and the Bentink et al. datasets. Thus the random-effects
analysis removed genes with strong effect in most training datasets, but no or
inverse estimated effect in either of these two studies. The random-effects model then
replaced these genes with ones that have slightly weaker, but more consistent
effect. 

To test the hypothesis that technical issues in these studies are the main
sources of heterogeneity, we used Integrative Correlation Analysis
\cite{Parmigiani:2004}.  This method tests whether a gene is correlated to
genes it is usually correlated to. A low integrative correlation of a gene
indicates unusual expression when compared to two reference datasets (Bonome
and TCGA). The reported results are the averaged results using once the Bonome
...
integrative correlation, or unusual expression, compared to the genes
...
both Yoshihara 2010 and the Bentink datasets. We see a trend in both datasets
that genes with heterogeneity have lower integrative correlation than genes
without, indicating that technical issues are at least partly a source of
heterogeneity. The difference was however not significant ($P =
...
of the individual signatures are overlapping.
...
enrichment analysis (build-in algorithm) was used to identify significant
pathways within the signature. A p-value less than 0.05 was considered
significant, indicating the associated pathway identification was unlikely to
be resulted from chance.
...
used for training. AUCs significantly ($P < 0.05$) larger than 0.5 are
...
a <- metaCMA.compare(fe.model, re.model, esets.f, coefs=coefs.debulking.s34b,
y="debulking")
@
...
As in section \ref{sec:fixed}, we examined the overlap of the gene signatures
obtained with a fixed- and a random-effects gene ranking (Supplemental
Figure~\ref{fig:effects:db:venn}). We further compared both fixed-effects and
random-effects signature with a signature obtained with the Rank Products
method \cite{Breitling:2004}, a frequently used alternative microarray
meta-analysis method. The Rank Products method currently does not support
censored outcome and could not be compared to the overall survival signatures.
The Rank Products signature was more similar to the fixed-effects (overlap of
114 genes) than to the random-effects signature (overlap of 76 genes), most
likely because Rank Product also weights datasets strictly according their
sample size.

We observed a slightly higher number of genes with heterogeneity in the
fixed-effects signature (\Sexpr{nhet} genes) compared to the overall survival
signature. This is again expected, since surgery outcome is not a clearly
defined phenotype in contrast to time to death. The Yoshihara 2010 and the
Bentink et al. datasets were again the most common source of
heterogeneity, albeit less clear than for the overall survival signature
(Supplemental Figures~\ref{fig:effects:db:hm1}-\ref{fig:effects:db:hm3}.

Integrative correlation analysis \cite{Parmigiani:2004} showed that there were
no systematic differences between the genes with significant heterogeneity and
the ones without (Supplemental Figure~\ref{fig:effects:db:intcor}).  We
expect, however, that the largest datasets (most importantly the Bonome
dataset published from our lab and the TCGA samples) are similar to our
validation samples in terms of surgical procedures and pathologic review.
Accounting for heterogeneity with a random-effects model will overweight
smaller studies for which we have only limited surgery information.
