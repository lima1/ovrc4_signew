...
patient samples - Supplementary Information}
...
cross-validation procedure (Supplementary Figure~\ref{fig:flowchart}). The goal
...
# to replace with the original Verhaak signature, use this 
# (we did not use this, because not all genes were available on all platforms, 
# the results are very similar though)
#x <- read.xls("input/JCI65833sd1.xls", sheet=7, skip=1)
#genesets <- lapply(levels(x$CLASS), function(y) { y <- as.character(x[x$CLASS==y,1]); y[y
#    %in% featureNames(esets.f[[1]])] } )
#names(genesets) <-  levels(esets.f[[1]]$tcga_subtype)
@
...
gene ranking is shown in Supplementary Figure~\ref{fig:effects:venn}. Out of
...
meta-analysis (Supplementary Figure~\ref{fig:effects:coef}).  Supplementary
...
heterogeneity (Supplementary Figure~\ref{fig:effects:hm2}). 

<<cached_short_intcor, cache=TRUE, results='hide'>>=
...
exclusively found in the random-effects signature (Supplementary
...
In Supplementary Figure~\ref{fig:cutoff:train} we confirm that the signature
...
to ensure the correctness of our model implementation (Supplementary
...
expression profile would have similar risk scores, however), but the main
...
subtype labels\footnote{\url{https://tcga-data.nci.nih.gov/tcgafiles/ftp_auth/distro_ftpusers/anonymous/other/publications/ov_exp/TCGA_489_UE.k4.txt}}. 
Final subtype scores were obtained by subtracting the ssGSEA
...
the Affymetrix data we used in our meta-analysis. 
...
<<cached_short_loadjci2, cache=TRUE, results='hide'>>=
x <- read.xls("input/JCI65833sd1.xls", skip=1)

# extract the official CLOVAR subtype scores and compare with our implementation
.getScores <- function(dataset, field="alt_sample_name", subtype="Mesenchymal") {
    idx.df <- grep(dataset, .getDatasetNames(esets.f))
    idx.v <- grep(dataset, x$DATASET, ignore.case=TRUE)
    idx.w <- grep(dataset, dfrcboth$batch, ignore.case=TRUE)

    if (field == "id") {
        ids <- match(make.names(x[idx.v,1]), sampleNames(esets.f[idx.df]))
    } else {
        ids <- match(tolower(x[idx.v,1]), gsub("ovarian cancer ", "", tolower(dfrcboth[idx.w, field])))
    }

    #tmp <- cbind(dfrcboth[idx.w,][ids,subtype], x[idx.v, paste(subtype,
    #".ssGSEA.normalized.score", sep="")])
    tmp <- cbind(gsva.res[[idx.df]][ match(subtype,
    rownames(gsva.res[[idx.df]])), ids], x[idx.v, paste(subtype,
    ".ssGSEA.normalized.score", sep="")])
    tmp <- tmp[complete.cases(tmp),]
    list(data.frame(tmp, Subtype=subtype), cor(tmp[,1], tmp[,2]))
}

d.f.verhaak <- do.call(rbind,unlist(lapply(c("Mesenchymal", "Immunoreactive"),
function(subtype) list(
    .getScores("Bonome", field="alt_sample_name", subtype)[[1]], 
    .getScores("Tothill", field="id", subtype)[[1]],
    .getScores("Yoshihara", field="id", subtype)[[1]], 
    .getScores("Dressman", field="id", subtype)[[1]]
)), recursive=FALSE))

verhaak.cor <-round( cor(d.f.verhaak[,1], d.f.verhaak[,2]), digits=2)
@
...
plot(d.f.verhaak$X1,d.f.verhaak$X2, col=as.factor(d.f.verhaak$Subtype), main="D)   Comparison of ssGSEA subtype scores", xlab="Our subtype score", ylab="Verhaak et al. subtype score"); legend("bottomright", levels(factor(d.f.verhaak$Subtype)),col=1:2, pch=1)
@
\caption{Association of subtype and overall survival. The model proposed by
Verhaak et al. \cite{Verhaak:2013} utilizes a subtype score and the purpose of
this figure is to demonstrate the correctness of our subtype model
implementation. (A) All training and validation datasets excluding TCGA. Note
the very similar survival curves in the corresponding figure 2B in Verhaak et
al. \cite{Verhaak:2013}. (B) Stratification of TCGA samples by subtype. (C)
Kaplan-Meier curves of the subtypes proposed by the Australian Ovarian Cancer
Study Group (AOCS) in Tothill et al. \cite{Tothill:2008}. This analysis
corresponds to Figure 5B of the Tothill study, with the difference that here
we show only the late-stage, high-grade, serous tumors used in our
meta-analysis.  (D) This scatterplot shows the high correlation of both the
Mesenchymal and Immunoreactive ssGSEA subtype scores from our implementation
and the official scores we extracted from the Verhaak et al. Supplement.  The
Mesenchymal and Immunoreactive scores are utilized in the Verhaak et al.
model. Shown are the scores for the Bonome, Dressman, Tothill and Yoshihara
samples.}
...
c1, which had poor prognosis in the Tothill data (Supplementary
...
Verhaak et al. provided ssGSEA scores for various public datasets in their
Supplement and we compared these scores with the scores of our implementation.
We found a high correlation of scores ($\rho = \Sexpr{verhaak.cor}$, Supplementary
Figure~\ref{fig:km:subtypesos}D). Note that a
perfect correlation is not expected, since we used a different pre-processing
protocol for the public data and utilized different probesets.
...
the multivariable predictions) are shown in Supplementary Figure~\ref{fig:cor:risk}. 
...
characteristics with known survival association. In Supplementary
...
(Supplementary Figure~\ref{fig:samplesize}). This finding demonstrated that (i)
...
meta-analysis summary is shown in Supplementary
...
\cite{Berchuck:2004} as in Supplementary Figure~\ref{fig:roc:debulking}. The
...
as in Supplementary Figure~\ref{fig:roc:debulking}.}
...
curves shown in Supplementary Figure~\ref{fig:roc:debulking:berchuck04}.
...
In Supplementary Table~\ref{tbl:reg:debulking}, we show the logistic regression
...
shown in Supplementary Figure~\ref{fig:roc:debulking:postn}. 
...
(Supplementary Figure~\ref{fig:effects:db:venn}). We further compared both
...
survival signature (Supplementary
...
heterogeneity and the ones without (Supplementary
...
associated with debulking status. See main text as well as Supplementary
...
In Supplementary Table~\ref{tbl:coefficients:ihc}, we show that POSTN, pSmad2/3
...
and unadjusted for stage and grade (shown in columns 1 and 2 of Supplementary
