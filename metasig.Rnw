...
shows a Kaplan-Meier analysis of the multi-variate model with the patients
stratified by median risk scores of the training data. The survival curves
shown in red correspond to the patients with highest risk scores, in blue to
the ones with lowest risk. Panel (b)
presents a forest plot of the prediction concordances.}
\end{figure}
...
.lodocvPlotForest(X,main="C-Index Meta-Analysis")
risk <- lapply(X, function(X) predict(model.tcga, newdata=X, type="lp")@lp)
for (i in 1:length(X)) X[[i]]$risk <- risk[[i]]
.lodocvPlotForest(X,main="C-Index TCGA")
@
\caption{
Comparison of the novel overall survival signature with the TCGA prediction
model. This forest plot shows the concordances of the risk predictions for
both models. For our model on the left (\textbf{Supplemental Table 1}), it uses the
leave-one-dataset-out cross-validated risk scores as in \textbf{Figure 1}. On the
right, we show the concordances of the risk scores estimated with the TCGA
model.
}
\end{figure}
...
See \textbf{Supplemental Figure~\ref{fig:reclass}} and \cite{Uno:2012} for an explanation of these
plots.}
\end{figure}
...
<<clinical>>=
    par(mfrow=c(2,2))
plotKMStratifyBy("Lau92",y=dfrc$y,linearriskscore=dfrc$age,censor.at=365.25*5, 
        show.n.risk=FALSE,show.legend=FALSE,show.HR=TRUE,cex.HR=0.85,
        xlab="Time (Days)",ylab="Overall Survival (%)",
        cex.base=1.4,main="A)   Age (>70 yes vs. no)")
plotKM(y=dfrc$y,strata=factor(dfrc$debulking_orig,levels=c("suboptimal", "optimal")),censor.at=365.25*5,
        show.n.risk=FALSE,show.legend=FALSE,show.HR=TRUE,cex.HR=0.85,
        xlab="Time (Days)",ylab="Overall Survival (%)",
        cex.base=1.4,main="B)   Debulking (subopt. vs opt.)")
plotKM(y=dfrc$y, strata=factor(dfrc$tumorstage,levels=c(4,3)),censor.at=365.25*5,
        show.n.risk=FALSE,show.legend=FALSE,show.HR=TRUE,cex.HR=0.85,
        xlab="Time (Days)",ylab="Overall Survival (%)",
        cex.base=1.4,main="C)   Stage (4 vs. 3)")
plotKMStratifyBy("median",y=dfrc$y, linearriskscore=dfrc$risk,censor.at=365.25*5,
        show.n.risk=FALSE,show.legend=FALSE,show.HR=TRUE,cex.HR=0.85,
        xlab="Time (Days)",ylab="Overall Survival (%)",
        cex.base=1.4,main="D)   Gene Signature")
@
\caption{Comparison of age, debulking status, stage and gene signature risk
score for all patients in the training datasets combined.
See caption in the main paper for details.}
\end{figure}

%<<>>=
% load("input/eset.early.scaled.rda")
% esets.early <- esets
%
%
%risk1 <- lapply(esets.early, function(X) predict(final.model, X)@lp)
%risk2 <- lapply(esets.early, function(X) predict(model.tcga, X)@lp)
%
%    names(esets.early) <- sapply(esets.early, function(Y)
%                gsub(" hgu95| Cannistra", "",
%                experimentData(Y)@lab ))
%
% x <- metaCMA.forest.models(esets.early, risks1=risk1, risks2=risk2)
%@
...
%risk1b <- lapply(esets.validation, function(X) predict(final.model, X)@lp)
%
%Xs.all <- c(Xs.all, esets.validation)
%risk1 <- c(risk1a, risk1b)
%
%risk2 <- lapply(Xs.all, function(X) predict(model.tcga, X)@lp)
%
%    names(Xs.all) <- sapply(Xs.all, function(Y)
%                gsub(" hgu95| Cannistra", "",
%                experimentData(Y)@lab ))
%
