...
patient samples: Biologic and Clinical Correlations - Supplemental Figures}
...
custom.names=c("Intercept", "Gene Signature","Stage (III vs IV)"))
@
...
and stage. We further validated 8 genes by qRT-PCR in 78 Bonome samples not used for training.
The selection criteria were (i) high rank by fold-change and FDR and (ii)
the results of our pathway analyses.  For example, PTCH1 was selected because
a validation would demonstrate another layour of evidence of the
