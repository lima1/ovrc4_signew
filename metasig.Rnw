...
\centering
\subfigure[]{
<<kmplotstrainclinical,cache=TRUE,fig.width=7,fig.height=4.66,out.width="0.9\\textwidth",out.height="0.6\\textwidth",results='hide'>>=
esets.r <- addClinicalAll(esets.f, ma[[10]]$fits)
idx <- sapply(esets.r, function(X) sum(!is.na(X$risk))> 20)
.lodocvPlot(esets.r[idx],nrow=2)
@
}\\
\subfigure[]{
...
signature combined with clinical factors debulking status and stage. Panel (a)
...
preds.logit.age <- .p2logitMV(Xs.all[idx], preds=preds[idx],
 fits=ma[[10]]$fits[idx],y="os_1yr",x1="tumorstage")
