...
Meta-Analysis, TCGA and Verhaak et al. signatures. 
Numbers in the upper-right, tringular half of the matrix
are the Pearson correlation coefficients, which were all statistically
significant ($P < 0.05$). Pairwise scatterplots of expression values are shown
in the lower-left half and the expression histograms are shown on the matrix
diagonal.}
\label{fig:cor:risk}
\end{figure}
...
tmp <- .createXy(Xs.alls34, coefs=coefs.debulking.s34,n=200,y="debulking")
...
\subfigure[]{
<<fpcomptcga,fig.width=5.75,fig.height=11.5,out.width="0.4\\textwidth",out.height="0.7\\textwidth",results='hide'>>=
