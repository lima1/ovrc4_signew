...
<<cached_medium_debulking_metas34, cache=TRUE, results='hide'>>=
...
dfrc.debulking$risk_os <-  unlist(lapply(names(Xs.alls34), function(id)  if
    (id %in% names(ma[[10]]$fits)) predict(ma[[10]]$fits[[id]]$fit,
        newdata=Xs.alls34[[id]])@lp else predict(final.model,
        newdata=Xs.alls34[[id]])@lp) )
