...
.plotN(esets.debulking.f[idx.debfi], yi, ylab="AUC")
@
\caption{We used for all our gene signatures a fixed gene signature size of
200 genes. Here we show the influence of this cutoff on the prediction
...
\clearpage
\section{qRT-PCR and IHC validation of genes associated with surgery outcome}

ROC curves for public microarray data of our top-ranked hit \textit{POSTN} are shown in Supplemental
Figure~\ref{fig:roc:debulking:postn}. We validated 8 genes by qRT-PCR in 78
Bonome samples not used for training. In Figure~\ref{fig:qrtpcr:cor}, we show
the correlation of the qRT-PCR expression values and the Affymetrix signal
intensities.

<<ihcpostn,fig.width=3.5,fig.height=3.5,include=FALSE>>=
ihc <- read.xls("input/IHC.xls", as.is=TRUE,  na.strings = "missing")

ihc$FIGO.x <- ihc$FIGO.Stage
ihc$FIGO.x[grepl("III", ihc$FIGO.Stage)] <- 3
ihc$FIGO.x[grepl("IV", ihc$FIGO.Stage)] <- 4
ihc$FIGO.x[grepl("II?", ihc$FIGO.x)] <- 2
ihc$FIGO.x[ihc$FIGO.x==""] <- NA
.plotROC(ihc$POSTN, as.factor(ihc$Debulking))
...
.plotROC(ihc$pSmad2, as.factor(ihc$Debulking))
@

<<ihcmultivariate,fig.width=3.5,fig.height=3.5,include=FALSE>>=
.plotROC(ihc$POSTN+ihc$pSmad2, as.factor(ihc$Debulking))
@

<<ihcmultivariatebar,fig.width=3.5,fig.height=3.5,include=FALSE>>=
x <- ihc$POSTN+ihc$pSmad2

 xq <- quantile(x, p=c(1/6,5/6),na.rm=TRUE)

 d.f <- t(table(ihc$Debulking, as.factor(ifelse(x <= xq[1], "Low Risk",
 ifelse(x>=xq[2], "High Risk", "Medium Risk")))))
 d.f <- rbind(data.frame(IHC=d.f[,1],Debulking="Optimal"),
    data.frame(IHC=d.f[,c(2)], Debulking="Suboptimal"))

 d.f$Group <- factor(rep(rownames(d.f)[1:3],2),levels=c("Low Risk", 
 "Medium Risk","High Risk"))
 ggplot(d.f, aes(Group, IHC,
 fill=Debulking))+geom_bar(stat="identity")+theme_classic()
@
...
<<debulking_corplot>>=
data(GSE26712_eset)
 tmp <- do.call(rbind, lapply(featureNames(eset.qrtpcr), function(g)
 data.frame(Gene=g,
...
facetAdjust(ggplot(tmp, aes(Affymetrix,
  ...
  parse=TRUE,  show_guide=FALSE)+theme_classic2())
@
...
represent patients, x-axis shows the Affymetrix fRMA normalized intensities,
the y-axis the qRT-PCR level. The Spearman correlation measurements was highly
significant in all genes ($\P < 0.001$).}
\label{fig:qrtpcr:cor}
\end{figure}

In the main paper, we show the grading of the tissue array staining for POSTN,
Supplemental Figure~\ref{fig:staining} shows the grading for pSmad2/3.

\begin{figure}
\centering
\includegraphics[width=0.8\textwidth]{input/psmadgrading}
\caption{Tissue array staining of pSmad2/3.}
\label{fig:staining}
\end{figure}
...
<<response>>=
data(TCGA_eset)
  TCGA_eset$response <- gsub("NA", NA,
  sapply(lapply(TCGA_eset$uncurated, function(x) strsplit(x,"///")[[1]]),
  function(y) gsub("primary_therapy_outcome_success: ", "", y[[54]])))

TCGA_eset$response <- as.factor(gsub(" ", "_", TCGA_eset$response))
 X <- TCGA_eset[, !is.na(TCGA_eset$response)]
