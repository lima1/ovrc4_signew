...
ma.db <- lapply(c(5,10,seq(25,250,25)), function(i)
    metaCMA(esets.debulking.f,coefs=coefs.debulking.s36b,n=i,y="debulking"))
@
...
ROC curves for our top-ranked hit \textit{POSTN} are shown in Supplemental
...
ihc <- read.xls("input/postnihc.xls", as.is=TRUE)
...
facetAdjust(ggplot(tmp, aes(Affymetrix,
  ...
  math_format(10^.x)))+theme_classic())
@
