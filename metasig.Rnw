...
plotKM(y=dfrc.subtypes$y, strata=dfrc.subtypes$tcga_subtype,censor.at=365.25*5,
        ...
        xlab="Time (Days)",ylab="Overall Survival (%)",
        cex.base=1.4,cex.legend=0.6,main="A) TCGA subtypes")
...
for (i in 1:4) {
    ...
    strata=dfrc.subtypes$strata[idx],censor.at=365.25*5,
            ...
            show.PV=FALSE,
            xlab="Time (Days)",ylab="Overall Survival (%)", cex.HR=0.8,
            cex.base=1.4,cex.legend=1.1,main=paste(toupper(letters[i+1]), ") ", tcga_subtype, sep=""))
}        
@
