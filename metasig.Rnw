...
dfrc.debulking <- .createClinical(esets.debulking.f)
...
dfrc.debulking$risk_os <-  unlist(lapply(names(esets.debulking.f), function(id)  if
    (id %in% names(ma[[10]]$fits)) predict(ma[[10]]$fits[[id]]$fit,
        newdata=esets.debulking.f[[id]])@lp else predict(final.model,
        newdata=esets.debulking.f[[id]])@lp) )
