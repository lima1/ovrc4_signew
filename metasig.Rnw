...
res.debulking.s3  <- metaCMA(Xs.alls3,y="debulking",coefs=coefs.debulking, n=200,
filter.fun=.debulkingFilter)
@
...
titles <- lapply(Xs.alls3, function(X) experimentData(X)@lab)
titles$TCGA_eset <- "TCGA 2011"
...
\begin{figure}
<<plotfpdebulking,fig.width=12,fig.height=6,out.width="0.9\\textwidth",out.height="0.45\\textwidth",results='hide'>>=
priors <- sapply(labels, function(l) sum(l=="suboptimal")/length(l))
 xxx <- .doTestAllPair(preds1=preds.logit,
  labels=labels[idx],
