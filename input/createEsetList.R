...
## 
##R CMD BATCH --vanilla "--args ../input/patientselection.config ../output/data/patientselection-esets.RData ../output/data/createEsetList.log" createEsetList.R 
## 
##./createEsetList.R ../input/patientselection.config ../output/data/patientselection-esets.RData ../output/data/createEsetList.log
...
if(length(inputArgs) > 0){
    ...
    kConfigFile <- "../input/patientselection.config"
    kOutputFile <- "../output/data/patientselection-esets.RData"
    kLogFile <- "../output/data/createEsetList.log"
...
for (strEset in strEsets){
    ...
    if (exists("combat") && combat) {
        # workaround bug #12
        if (pubMedIds(eset) == "21720365") {
            eset$batch[match("TCGA.23.1023", sampleNames(eset))] <- 12
        }
        if (sum(is.na(eset$batch)) == 0) {
            loginfo(paste("Running ComBat on", strEset))
            tmp <- try(sva::ComBat(exprs(eset),
                mod=model.matrix(~rep(1, ncol(eset))), batch=eset$batch))
            if (class(tmp)=="matrix") {
                exprs(eset) <- tmp
            }
        } 
