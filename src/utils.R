...
.lodocvPlotForest <- function(X, main="C-Index") {
    ...
    metaCMA.forest.label(esets=X,label="risk",y="y",concordance=TRUE,cex=0.7,main=main,
    xlab="")
}
...
.plotROC <- function(pred, labels,plot=TRUE,na.rm=TRUE,...) {
    ...
    if (plot) {
        ...
        text(0,0.9,paste("AUC =",
            round(auc,digits=2)),cex=1.5,pos=4)
            abline(a=0,b=1, lty=2)
        text(1,0.1,paste("n =",length(labels))
            ,cex=1.5,pos=2)
            abline(a=0,b=1, lty=2)
    }
    invisible(auc)
...
.p2logit <- function(esets, fits, preds=NULL, y="debulking") {
     .doIt <- function(i) {
        ...
        tmp.df <- data.frame(risk=predict(fits[[i]]$fit,
        newdata=esets.c$X)@lp,covar=esets.c$y)
        fit <- glm(covar~risk,tmp.df,family="binomial")
        risk <- fits[[i]]$risk@lp
        if (!is.null(preds)) risk <- preds[[i]]
...
.p2logitMV <- function(esets, fits, preds=NULL, y="debulking",x1="tumorstage") {
     .doIt <- function(i) {
        esets.c <- .combineEsets(esets[-i],y=y)
        tmp.df <- data.frame(risk=predict(fits[[i]]$fit,
        newdata=esets.c$X)@lp,covar=esets.c$y, x1=
        unlist(sapply(esets[-i], function(X) X[[x1]]))
        )
...
.doTestAll <- function(preds, labels, priors, titles, method="FE") {
    ci <-
    do.call(rbind,lapply(1:length(preds),function(i)try(.doTest(preds[[i]],labels[[i]],prior=priors[i]))))
    rownames(ci) <-titles
    se <-
    do.call(rbind,lapply(1:length(preds),function(i)try(.doTestSE(preds[[i]],labels[[i]],prior=priors[i]))))
    rownames(se) <-titles

    rma.res <- lapply(seq(1,9,2), function(i) metafor::rma(yi=se[,i],
    sei=se[,i+1], method=method))
    .fp <- function(p) round(p*100,digits=1)
    .fnp <- function(p) round(p,digits=2)
    pooled1 <- lapply(rma.res[-5], function(x) c(.fp(x$b), paste(.fp(x$ci.lb),
                .fp(x$ci.ub), sep="-")))
    pooled2 <- lapply(rma.res[5], function(x) c(.fnp(x$b), paste(.fnp(x$ci.lb),
                .fnp(x$ci.ub), sep="-")))
...
.doTestSE <- function(preds, labels, prior, cutoff=quantile(preds,p=(1-prior) )) { 
    ...
    .fp <- function(p) p
    .fnp <- function(p) p
    data.frame(PPV=.fp(tbl[2,2]/sum(tbl[2,])),
    PPV.SE=diff(binom.test(tbl[2,2],sum(tbl[2,]))$conf.int)/3.92,

    NPV=.fp(tbl[1,1]/sum(tbl[1,])),
    NPV.SE=diff(binom.test(tbl[1,1],sum(tbl[1,]))$conf.int)/3.92,
    FPR=.fp(tbl[2,1]/sum(tbl[,1])),
    FPR.SE=diff(binom.test(tbl[2,1],sum(tbl[,1]))$conf.int)/3.92,
    FNR=.fp(tbl[1,2]/sum(tbl[,2])),
    FNR.SE=diff(binom.test(tbl[1,2],sum(tbl[,2]))$conf.int)/3.92,
    OR=.fnp(f$estimate), 
    OR.SE=diff(f$conf.int)/3.92, stringsAsFactors=FALSE)
}

.doTest <- function(preds, labels, prior,
cutoff=quantile(preds,p=(1-prior) )) { 
    ...
    .fnp <- function(p) round(p,digits=2)
    data.frame(PPV=.fp(tbl[2,2]/sum(tbl[2,])),
    PPV.CI=paste(sapply(binom.test(tbl[2,2],sum(tbl[2,]))$conf.int, .fp),collapse="-"),

    NPV=.fp(tbl[1,1]/sum(tbl[1,])),
    NPV.CI=paste(sapply(binom.test(tbl[1,1],sum(tbl[1,]))$conf.int, .fp),collapse="-"),
    FPR=.fp(tbl[2,1]/sum(tbl[,1])),
    FPR.CI=paste(sapply(binom.test(tbl[2,1],sum(tbl[,1]))$conf.int, .fp),collapse="-"),
    FNR=.fp(tbl[1,2]/sum(tbl[,2])),
    FNR.CI=paste(sapply(binom.test(tbl[1,2],sum(tbl[,2]))$conf.int, .fp),collapse="-"),
    OR=.fnp(f$estimate), OR.CI=paste(sapply(f$conf.int,
...
.p2logitStage <- function(esets, fits, y="debulking") {
     .doIt <- function(i) {
        ...
        intercept.se <- do.call(rbind,lapply(gfits,
            function(x) summary(x)$coefficients[1,]))
        risk.se <- do.call(rbind,lapply(gfits,
            function(x) summary(x)$coefficients[2,]))
        idx <- sapply(gfits,function(x)
            nrow(summary(x)$coefficients)==3)
        T.se <- do.call(rbind,lapply(gfits[idx],
            function(x) summary(x)$coefficients[3,]))
        res <- list(gfits,  
            intercept = metafor::rma(yi= intercept.se[,1], sei= intercept.se[,2])$b,
            br = metafor::rma(yi= risk.se[,1], sei= risk.se[,2])$b,
            bt = metafor::rma(yi= T.se[,1], sei= T.se[,2])$b)
...
.p2logitStage2 <- function(esets, fits, y="debulking") {
     .doIt <- function(i) {
        esets.c <- .combineEsets(esets[-i],y=y)
        tmp.df <- data.frame(risk=predict(fits[[i]]$fit,
        newdata=esets.c$X)@lp,covar=esets.c$y, tumorstage=
        as.factor(unlist(sapply(esets[-i], function(X) X$tumorstage)))
        )
        fit <- glm(covar~risk+tumorstage,tmp.df,family="binomial")
        predict(fit,newdata=data.frame(risk=fits[[i]]$risk@lp,tumorstage=as.factor(esets[[i]]$tumorstage)),type="response")
    } 
    lapply(1:length(esets), .doIt)
}
...
.reclassPlot <- function(y, r1, r2, t0, title, plot=TRUE) {
    ...
    y  <- IDI.INF.OUT(x)
    if (plot) {
    IDI.INF.GRAPH(x, main=paste(title, " (t0 = ", t0, ")",
    sep=""),las=1,cex.lab=1.3)
    text(par("usr")[1],0.95, paste("IDI = ", y[1,1], "; 95% CI ", y[1,2]," to ",
     y[1,3],sep="") , pos=4,cex=1.45)
   }  
   y
}
...
skip=0,dataset_ids,ylab="C-Index (Concordance)",panel.num="A",...) {
    ...
    # exclude TCGA, because we compare concordance to TCGA model throughout
    # the paper
...
.createClinical <- function(esets, clinical.covars=NULL) {
   if (is.null(clinical.covars)) clinical.covars <- c("age_at_initial_pathologic_diagnosis", "debulking",
   "tumorstage", "days_to_death", "vital_status")
   df <- data.frame(do.call(cbind,lapply(clinical.covars, function(covar)
            unlist(sapply(esets, function(X) X[[covar]])))),  stringsAsFactors = FALSE)
   colnames(df) <- clinical.covars         
   df$tumorstage <- as.numeric(df$tumorstage)
   df$batch <-  as.factor(unlist(sapply(esets, function(X)
   rep(experimentData(X)@lab,ncol(X)))))
...
addClinical <- function(idx, esets, fits) {
    ...
    df$risk <- unlist(sapply((1:length(esets))[-idx], .getRiskScore, idx, esets, fits))
    fit =
    coxph(y~as.factor(age_at_initial_pathologic_diagnosis>70)+debulking+risk,data=df) 
...
addClinicalBinary <- function(idx, esets, fits) {
    df <-
    .createClinical(esets[-idx],clinical.covars=c("age_at_initial_pathologic_diagnosis","debulking","tumorstage","days_to_death","vital_status","oneyr"))
    df$risk <- unlist(sapply((1:length(esets))[-idx], .getRiskScore, idx, esets, fits))
    fit =
    glm(as.logical(oneyr)~as.factor(age_at_initial_pathologic_diagnosis>60)+debulking+risk,data=df,family="binomial") 
}
...
addClinicalBinaryBK <- function(idx, esets, fits) {
    ...
    df$risk <- unlist(sapply((1:length(esets))[-idx], .getRiskScore, idx, esets, fits))
    fit =
    glm(df$debulking=="suboptimal"~as.factor(age_at_initial_pathologic_diagnosis>60)+debulking+risk,data=df,family="binomial") 
}

addClinicalAll <- function(esets, fits) {
    res = lapply(1:length(esets), addClinical, esets, fits)
    esets.r = lapply(1:length(esets), function(i) { esets[[i]]$risk = res[[i]]$risk;
    esets[[i]] } )
